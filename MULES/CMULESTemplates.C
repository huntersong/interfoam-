...
void Foam::MULES::correct      //返回修正场
(                                       // correct是函数名 （参数列表）  {函数体} 
    const RdeltaTType& rDeltaT,     //const修饰函数的参数 自定义类型的参数传递 初始化。即 const 成员函数不能修改成员变量的值，但可以访问成员变量
    const RhoType& rho,              //加入const 不希望correct函数去修改原始数据
    volScalarField& psi,
...
void Foam::MULES::correct        //以下correct都是 函数重载 对psi参数进行修正 psi对应物理量alpha1
        //函数的重载：函数名必须相同，参数列表必须不同（个数不同、类型不同、参数排列顺序不同），函数返回类型可以相同也可以不同
        //仅仅返回类型不同不足以成为函数的重载
(
    const RhoType& rho,
    volScalarField& psi,
...
void Foam::MULES::correct
(
    const RhoType& rho,			//geometricOneField()
    volScalarField& psi,		//alpha1
    const surfaceScalarField& phi,	//talphaPhi()
    surfaceScalarField& phiCorr,	//talphaPhiCorrr.ref()
    const SpType& Sp,			//	
    const SuType& Su,			//
    const PsiMaxType& psiMax,  		// 1
    const PsiMinType& psiMin   		// 0 
)
{
    const fvMesh& mesh = psi.mesh();
...
)
{
    ...
    forAll(phiCorrIf, facei)   //求每个cell附近的最大值，并记录每个cell的P+-i
    {
    ...
    for (int j=0; j<nLimiterIter; j++)      //迭代求解lambdafvSolution中nLimiterIter    5;
    {
        sumlPhip = 0;
        mSumlPhim = 0;
        ...
        forAll(lambdaIf, facei)    //根据Af通量的方向 更新上述的lambda
        {
...
template          //计算带限制器的anti-diffusive flux的函数
<
    class RdeltaTType,
    class RhoType,
    class SpType,
